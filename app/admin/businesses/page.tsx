        ...
        <table className="w-full min-w-[560px] text-left text-sm">
          <thead>
            <tr className="border-b border-line bg-paper text-xs uppercase tracking-wide text-muted">
              <th className="px-3 py-3 sm:px-5 font-medium">Business</th>
              <th className="px-3 py-3 sm:px-5 font-medium">Category</th>
              <th className="px-3 py-3 sm:px-5 font-medium">Owner login</th>
          ...
          <tbody>
            {rows === null && !error && (
              <tr>
                <td colSpan={4} className="px-3 py-6 sm:px-5 text-center text-muted">
                  Loading…
                </td>
              </tr>
            )}
            ...
            {rows?.map((b) => (
              <tr key={b.id} className="border-b border-line last:border-0 hover:bg-paper/60">
                <td className="px-3 py-3.5 sm:px-5 font-medium text-ink">{b.name}</td>
                <td className="px-3 py-3.5 sm:px-5 text-muted">{b.category ?? '—'}</td>
                <td className="px-3 py-3.5 sm:px-5 text-muted">{b.ownerEmail ?? 'Not yet invited'}</td>
